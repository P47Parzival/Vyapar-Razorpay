...
async function askLlmToPick(client, goal, catalogJson, validIds) {
...
Pick the single best matching item. Respond with only its id.`;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (idMatch && validIds.has(idMatch[1]))
                return idMatch[1];
...
async function main() {
    ...
    const insertTrial = db.prepare(`INSERT INTO catalog_trials (id, goal, goal_id, trial_number, catalog_snapshot_order_json, picked_item_id, run_batch_id)
     VALUES (?, ?, ?, ?, ?, ?, ?)`);
    let totalTrials = 0;
    let validPicks = 0;
    let nullPicks = 0;
    for (const goal of goals) {
        ...
        for (let trial = 1; trial <= TRIALS_PER_GOAL; trial++) {
            ...
            insertTrial.run(`trial_${randomUUID().slice(0, 8)}`, goal.goal, goal.id, trial, JSON.stringify(snapshotOrder), pickedId, batchId);
            totalTrials++;
            if (pickedId) {
                validPicks++;
    ...
    const rowCount = db.prepare('SELECT COUNT(*) as n FROM catalog_trials WHERE run_batch_id = ?').get(batchId).n;
