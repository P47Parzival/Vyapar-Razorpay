...
export async function parseWhatsAppPolicyMessage(messageText) {
    try {
        const response = await callLlm(SYSTEM_PROMPT, [{ role: 'user', content: [{ text: messageText }] }], TOOLS);
        ...
        return {
            type: 'parse_failure',
            reason: input.reason || 'Message does not contain a recognizable policy change request.',
        };
    }
    catch (err) {
