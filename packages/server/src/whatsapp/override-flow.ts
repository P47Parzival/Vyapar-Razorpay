...
import { processProposal, type ProcessOptions } from '../gateway/policy-gateway.js';
...
export async function executeOverride(
  ...
  const ledgerRow = db.prepare(
    `SELECT * FROM ledger WHERE merchant_id = ? AND json_extract(proposal_json, '$.proposal_id') = ? ORDER BY timestamp DESC LIMIT 1`
  ).get(merchantId, proposalId) as LedgerRow | undefined;

  if (!ledgerRow) {
    const reply = `Could not find proposal ${proposalId}. Check the ID and try again.`;
    insertAuditLog.run(
      `wa_override_${randomUUID().slice(0, 8)}`,
      merchantId, from, originalMessage,
      JSON.stringify({ type: 'single_use_override', proposal_id: proposalId, action: 'approve' }),
      'deferred', 'single_use_override', null, null, reply
    );
  ...
  const existingOverride = db.prepare(
    `SELECT id FROM single_use_overrides WHERE proposal_id = ? AND merchant_id = ?`
  ).get(proposalId, merchantId) as { id: string } | undefined;
  ...
  const originalProposal = JSON.parse(ledgerRow.proposal_json) as Proposal;
  const decision = JSON.parse(ledgerRow.decision_json) as Decision;
  ...
  const overrideOptions: ProcessOptions = {
    singleUseOverride: {
  ...
  try {
    ...
    if (result.outcome.final_status === 'executed' || result.outcome.final_status === 'error') {
      ...
      insertAuditLog.run(
        ...
        merchantId, from, originalMessage,
        JSON.stringify({ type: 'single_use_override', proposal_id: proposalId, action: 'approve' }),
        'auto_applied', 'single_use_override',
        `denied (${decision.reason_code})`, `${result.outcome.final_status} (override)`,
        reply
      );
      ...
      insertAuditLog.run(
        ...
        merchantId, from, originalMessage,
        JSON.stringify({ type: 'single_use_override', proposal_id: proposalId, action: 'approve' }),
        'deferred', 'single_use_override',
        `denied (${decision.reason_code})`, `denied (${result.decision.reason_code})`,
        reply
      );
