...
export function checkAndNotifyNotableDenial(
  proposal: Proposal,
  decision: Decision
): void {
  if (decision.verdict !== 'denied') return;
  const MERCHANT_NUMBER = process.env.MERCHANT_WHATSAPP_NUMBER || '';
  if (!MERCHANT_NUMBER) return;

  const isCapOrVelocity =
    decision.reason_code === 'PER_TRANSACTION_CAP_EXCEEDED' ||
    decision.reason_code === 'VELOCITY_CAP_EXCEEDED';

  if (!isCapOrVelocity) return;
  ...
  if (proposal.amount_paise < threshold) return;
  ...
  insertAuditLog.run(
    `wa_notify_${randomUUID().slice(0, 8)}`,
    proposal.merchant_id,
    'system',
    `[Outbound] Notable denial: ₹${amountRupees} > 2x cap ₹${capRupees}`,
    JSON.stringify({ proposal_id: proposal.proposal_id, amount_paise: proposal.amount_paise, reason_code: decision.reason_code }),
