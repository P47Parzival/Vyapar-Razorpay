...
export function seedDatabase() {
  ...
  const existingMerchants = db.prepare('SELECT COUNT(*) as count FROM merchants').get() as { count: number };
  ...
  const existingItems = db.prepare("SELECT COUNT(*) as count FROM catalog_items WHERE merchant_id = 'default'").get() as { count: number };
  ...
  const existingM2Items = db.prepare("SELECT COUNT(*) as count FROM catalog_items WHERE merchant_id = 'merchant_2'").get() as { count: number };
  ...
  const existingPolicy = db.prepare("SELECT COUNT(*) as count FROM policy_config WHERE merchant_id = 'default'").get() as { count: number };

  if (existingPolicy.count === 0) {
    db.prepare(
      `INSERT INTO policy_config (merchant_id, max_per_transaction_paise, max_daily_velocity_paise, max_daily_txn_count, discount_ceiling_pct, mandate_expiry_minutes, merchant_allowlist_json, category_allowlist_json)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      defaultPolicy.merchant_id,
      defaultPolicy.max_per_transaction_paise,
      defaultPolicy.max_daily_velocity_paise,
      defaultPolicy.max_daily_txn_count,
      defaultPolicy.discount_ceiling_pct,
      defaultPolicy.mandate_expiry_minutes,
      defaultPolicy.merchant_allowlist_json,
      defaultPolicy.category_allowlist_json
    );
    console.log('[Seed] Inserted default policy config');
  }

  // Seed policy — merchant 2 (distinct caps)
  const existingM2Policy = db.prepare("SELECT COUNT(*) as count FROM policy_config WHERE merchant_id = 'merchant_2'").get() as { count: number };
  ...
  if (existingM2Policy.count === 0) {
    db.prepare(
      ...
      merchant2Policy.merchant_id,
      merchant2Policy.max_per_transaction_paise,
      merchant2Policy.max_daily_velocity_paise,
      merchant2Policy.max_daily_txn_count,
      merchant2Policy.discount_ceiling_pct,
      merchant2Policy.mandate_expiry_minutes,
      merchant2Policy.merchant_allowlist_json,
      merchant2Policy.category_allowlist_json
    );
  ...
  if (existingMandates.count === 0) {
    db.prepare(
      `INSERT INTO mandates (id, merchant_id, agent_id, principal, granted_at, expires_at, revoked, scope_max_amount_paise, scope_category_json, issued_by, consent_method)
       VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`
    ).run('mandate_growth_001', 'default', 'growth', 'merchant_default', now, expiresAt,
      300000, '["skincare","haircare","bodycare","wellness","accessories"]', 'system', 'dev_seed');

    db.prepare(
      `INSERT INTO mandates (id, merchant_id, agent_id, principal, granted_at, expires_at, revoked, scope_max_amount_paise, scope_category_json, issued_by, consent_method)
       VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`
    ).run('mandate_buyer_001', 'default', 'buyer', 'merchant_default', now, expiresAt,
      300000, '["skincare","haircare","bodycare","wellness","accessories"]', 'system', 'dev_seed');

    db.prepare(
      `INSERT INTO mandates (id, merchant_id, agent_id, principal, granted_at, expires_at, revoked, scope_max_amount_paise, scope_category_json, issued_by, consent_method)
       VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`
    ).run('mandate_growth_m2_001', 'merchant_2', 'growth', 'merchant_2_owner', now, expiresAt,
      500000, '["apparel","electronics"]', 'system', 'dev_seed');

    db.prepare(
      `INSERT INTO mandates (id, merchant_id, agent_id, principal, granted_at, expires_at, revoked, scope_max_amount_paise, scope_category_json, issued_by, consent_method)
       VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`
    ).run('mandate_buyer_m2_001', 'merchant_2', 'buyer', 'merchant_2_owner', now, expiresAt,
      500000, '["apparel","electronics"]', 'system', 'dev_seed');
