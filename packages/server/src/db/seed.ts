...
export function seedDatabase() {
  const existingItems = db.prepare('SELECT COUNT(*) as count FROM catalog_items').get() as { count: number };

  if (existingItems.count === 0) {
    const insertItem = db.prepare(
      'INSERT INTO catalog_items (id, title, description, price_paise, category, stock, pairs_with_ids) VALUES (?, ?, ?, ?, ?, ?, ?)'
    );
  ...
  const existingPolicy = db.prepare('SELECT COUNT(*) as count FROM policy_config').get() as { count: number };
