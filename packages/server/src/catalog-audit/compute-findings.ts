...
interface TrialRow {
  ...
  goal_id: string;
  trial_number: number;
  catalog_snapshot_order_json: string;
  picked_item_id: string | null;
  run_batch_id: string;
}

interface PickRateEntry {
  item_id: string;
  item_title: string;
  category: string;
  times_picked: number;
...
interface PositionEntry {
  ...
  times_picked: number;
  avg_position: number;
  positions_when_picked: number[];
}

export interface GoalFindings {
  goal_id: string;
  goal_text: string;
  total_trials: number;
  valid_picks: number;
  null_picks: number;
  pick_rates: PickRateEntry[];
  never_picked: NeverPickedEntry[];
  position_analysis: {
...
export interface BatchFindings {
  run_batch_id: string;
  total_trials: number;
  total_goals: number;
  catalog_size: number;
...
export function computeFindings(batchId: string): BatchFindings {
  const trials = db.prepare(
    'SELECT * FROM catalog_trials WHERE run_batch_id = ? ORDER BY goal_id, trial_number'
  ).all(batchId) as TrialRow[];
  ...
  const goalGroups = new Map<string, TrialRow[]>();
  ...
  for (const [goalId, goalTrials] of goalGroups) {
    ...
    const neverPicked: NeverPickedEntry[] = activeItems
    ...
    const positionData = new Map<string, number[]>();

    for (const t of validTrials) {
      const snapshotOrder: string[] = JSON.parse(t.catalog_snapshot_order_json);
      const position = snapshotOrder.indexOf(t.picked_item_id!);
      if (position === -1) continue;

      if (position < thirdSize) topThird++;
      else if (position < thirdSize * 2) middleThird++;
      else bottomThird++;
...
function printFindings(findings: BatchFindings) {
  ...
  for (const g of findings.goals) {
    ...
    const grouped = new Map<string, NeverPickedEntry[]>();
...
function getLatestBatchId(): string | null {
  const row = db.prepare(
    'SELECT run_batch_id FROM catalog_trials ORDER BY created_at DESC LIMIT 1'
