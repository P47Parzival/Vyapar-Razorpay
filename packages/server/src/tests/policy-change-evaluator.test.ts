import { evaluatePolicyChangeRequest, type PolicyFieldChange, type SingleUseOverride } from '../whatsapp/policy-change-evaluator.js';
...
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'per_transaction_cap', to: 1500 };
...
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'per_transaction_cap', to: 2000 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'auto_apply', `Decision: ${result.decision}`);
}
...
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'per_transaction_cap', to: 5000 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
...
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'category_allowlist', to: 0 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'defer_to_dashboard', `Decision: ${result.decision}`);
...
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'per_transaction_cap', to: 500 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'auto_apply', `Decision: ${result.decision}`);
...
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'per_transaction_cap', to: 100 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'defer_to_dashboard', `Decision: ${result.decision}`);
...
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'discount_ceiling', to: 20 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'auto_apply', `Decision: ${result.decision}`);
}
...
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'discount_ceiling', to: 60 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'defer_to_dashboard', `Decision: ${result.decision}`);
...
console.log('\n9. Daily velocity cap — small increase');
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'daily_velocity_cap', to: 15000 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'auto_apply', `Decision: ${result.decision}`);
}
...
console.log('\n11. No-op — same value');
{
  const change: PolicyFieldChange = { type: 'policy_field_change', field: 'per_transaction_cap', to: 1000 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'auto_apply', `Decision: ${result.decision}`);
...
{
  const change: SingleUseOverride = { type: 'single_use_override', proposal_id: 'prop_abc123', action: 'approve' };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'auto_apply', `Decision: ${result.decision}`);
}
...
{
  const change: SingleUseOverride = { type: 'single_use_override', proposal_id: 'prop_abc123', action: 'approve', discount_pct: 25 };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'defer_to_dashboard', `Decision: ${result.decision}`);
...
{
  const change: SingleUseOverride = { type: 'single_use_override', proposal_id: '', action: 'approve' };
  const result = evaluatePolicyChangeRequest(change, basePolicy);
  assert(result.decision === 'defer_to_dashboard', `Decision: ${result.decision}`);
}
