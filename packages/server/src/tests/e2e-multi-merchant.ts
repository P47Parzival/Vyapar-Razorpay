...
async function run() {
  ...
  const sorted = allItems.every((item, i) => {
    ...
    if (prev.category < item.category) return true;
    if (prev.category === item.category) return prev.price_paise <= item.price_paise;
    return false;
  });
  ...
  console.log('\n3. Independent policy configs');
  const p1 = db.prepare('SELECT * FROM policy_config WHERE merchant_id = ?').get('default') as any;
  const p2 = db.prepare('SELECT * FROM policy_config WHERE merchant_id = ?').get('merchant_2') as any;
  ...
  db.prepare(
    `INSERT INTO mandates (id, merchant_id, agent_id, principal, granted_at, expires_at, revoked, scope_max_amount_paise, scope_category_json, issued_by, consent_method)
     VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`
  ).run(m1MandateId, 'default', 'buyer', 'e2e_test', now, testExpiry,
    300000, '["skincare","haircare","bodycare","wellness","accessories"]', 'e2e_test', 'e2e_test');

  const m2MandateId = `mandate_e2e_m2_${randomUUID().slice(0, 8)}`;
  db.prepare(
    `INSERT INTO mandates (id, merchant_id, agent_id, principal, granted_at, expires_at, revoked, scope_max_amount_paise, scope_category_json, issued_by, consent_method)
     VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`
  ).run(m2MandateId, 'merchant_2', 'buyer', 'e2e_test', now, testExpiry,
    500000, '["apparel","electronics"]', 'e2e_test', 'e2e_test');

  const m1Mandate = db.prepare('SELECT * FROM mandates WHERE id = ?').get(m1MandateId) as any;
  const m2Mandate = db.prepare('SELECT * FROM mandates WHERE id = ?').get(m2MandateId) as any;
  ...
  const ledgerEntry = db.prepare('SELECT * FROM ledger WHERE id = ?').get(result.ledgerRow.id) as any;
  ...
  // ── 7. Order attribution ──
  console.log('\n7. Order attribution');
  const order = db.prepare("SELECT * FROM orders WHERE merchant_id = 'merchant_2' ORDER BY created_at DESC LIMIT 1").get() as any;
