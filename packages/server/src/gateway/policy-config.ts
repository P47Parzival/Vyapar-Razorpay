...
export function getPolicyConfig(merchantId: string = 'default'): PolicyConfig {
  const row = db.prepare('SELECT * FROM policy_config WHERE merchant_id = ?').get(merchantId) as PolicyRow;
  return {
...
export function updatePolicyConfig(merchantId: string, updates: Partial<Omit<PolicyConfig, 'merchant_id' | 'updated_at' | 'merchant_allowlist' | 'category_allowlist'>> & { merchant_allowlist?: string[]; category_allowlist?: string[] }): PolicyConfig {
  ...
  db.prepare(
    ...
    newConfig.max_per_transaction_paise,
    newConfig.max_daily_velocity_paise,
    newConfig.max_daily_txn_count,
    newConfig.discount_ceiling_pct,
